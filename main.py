...
def update():
    X_test = pd.DataFrame(
        np.array([(area, perimeter, major_axis_length, minor_axis_length, eccentricity, convex_area, extent)]),
        columns=cache['columns'])
    X_test_scaled = pd.DataFrame(cache['scaler'].transform(X_test), columns=cache['columns'])
...
area = st.sidebar.slider(label=cache['columns'][0], min_value=7000, max_value=20000, step=200, value=10000)
perimeter = st.sidebar.slider(label=cache['columns'][1], min_value=300, max_value=600, step=20, value=380)
major_axis_length = st.sidebar.slider(label=cache['columns'][2], min_value=100, max_value=300, step=10, value=160)
minor_axis_length = st.sidebar.slider(label=cache['columns'][3], min_value=50, max_value=120, step=10, value=80)
eccentricity = st.sidebar.slider(label=cache['columns'][4], min_value=0.7, max_value=1.0, step=0.05, value=0.85)
